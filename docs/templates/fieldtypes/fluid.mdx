...
:::note
The following examples use a field named `fluid_field`. This should be replaced with your field name.
:::

## Template Variables

When iterating over the rows of a Fluid field in a template you will have access to each field but also extra variables which can be used to control the display of these fields and field groups. It is important to remember that each `row` behaves as a field of it's own.  So the variables, parameters, and modifiers available on the `row` will depend on the fieldtype being used.

<Tabs>
<TabItem value="twig" label="Twig">

```twig
{% for row in entry.fluid_field %}
    Field Value: {{ row }}
...
```php
@foreach($entry->fluid_field as $row)
    Field Value: {{ $row }}
...
The following example assumes the Fluid Field has a Field Group named `my_group` which contains a [Text](./text.mdx) field `my_text`, a [File](./file.mdx) field `my_image` and a [Grid](./grid.mdx) field `my_grid`.
